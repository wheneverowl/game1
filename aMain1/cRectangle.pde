class Rectangle {
  ...
  public Rectangle(int setRectangleX, int setRectangleY, int setRectangleWidth, int setRectangleHeight, boolean IsPlatform, color SetColour) {
    ...
    this.Colour = SetColour; //sets values based on parameters, x,y for intial where to start rectangle and platform is whether a platform or not. Colour
  }
  ...
  } //draws rectangle according to colour defined and dimensions defined

  public void collidePlayerRectangle() {
    // creates rectangle that are top, bottom, left and right according to proprotions
    // then based on when the collision occurs and what rectangle it was on, it does a colliision calculation for that side.
